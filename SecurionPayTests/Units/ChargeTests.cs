...
namespace SecurionPayTests.Units
{
    ...
    public class ChargeTests:BaseUnitTestsSet
    {
        ...
        public async Task CreateChargeWithTokenTest()
        {
            ...
            var customerId = "1";
            var tokenId = "1";
            var chargeRequest = new ChargeRequest() { Amount = 1000, CurrencyISOCode = "PLN", Card = ChargeCardDefinition.FromCardToken(tokenId) , Description = "sss", Captured = false };
        ...
        public async Task CreateChargeWithCardTest()
        {
            ...
            var chargeRequest = new ChargeRequest() { Amount = 2000, CurrencyISOCode = "EUR", CustomerId = customerId, Card = ChargeCardDefinition.NewCard(cardRequest) };
            await requestTester.TestMethod<Charge>(
                async (api) =>
                {
                    await api.CreateCharge(chargeRequest);
